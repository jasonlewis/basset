...
class Collection {
	...
	public function add($name)
	{
		...
		elseif (File::exists(path('public').$name))
		{
			$assetPath = path('public').$name;
		}

		// Add an asset that's within one of the configured directories.
		else
		{
			foreach (Config::get('basset::basset.directories') as $directory => $path)
			{
	...
	{
		return File::exists($this->getCompilingPath().'/'.$this->getCompiledName($group));
	}
	...
	{
		return path('public').Config::get('basset::basset.compiling_path');
	}
	...
	public function parseDirectory($directory)
	{
		...
		elseif (starts_with($directory, 'name: '))
		{
			...
			if (Config::has("basset::basset.directories.{$directory}"))
			{
				$directory = Config::get("basset::basset.directories.{$directory}");
			}
